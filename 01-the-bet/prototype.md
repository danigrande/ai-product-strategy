# The Prototype Bet

## What I Built
<!-- One sentence: what does this prototype demonstrate? -->
A functional prototype of the World Cup pool with an AI agent

## Tool Used
<!-- v0 / Cursor / Lovable / other -->
Antigravity

## Prototype Link
<!-- Paste the shareable URL -->
https://porra-mundial-six.vercel.app/
https://testflight.apple.com/join/dm43b6bp

## AI Value Archetype
<!-- Automator / Copilot / Oracle / Creator / Orchestrator -->
Copilot

## The Bet in One Sentence
<!-- What you're building, for whom, why now -->
I'm building platform to make world cup predictions in which users can interact with each other and an AI agent. 

## Kill Criteria
<!-- When would you stop? What evidence would kill this bet? --> When the cost of infrastructure is higher than expected revenue.
If the GTM cost is bigger than 500 euros.
If the amount of users is less than 2000 after X amount of time and there is no clear traction.
